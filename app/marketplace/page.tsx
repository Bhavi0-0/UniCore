...
const mockProducts = [
  ...
  { id: 3, title: "Lab Coat (Size M)", price: 250, condition: "Good", seller: "Rahul V.", category: "Clothing", icon: Shirt },
  { id: 4, title: "Data Structures Book (Cormen)", price: 450, condition: "Fair", seller: "Sneha G.", category: "Books", icon: BookOpen },
  { id: 5, title: "Drafter Set & Drawing Board", price: 600, condition: "New", seller: "Vikram S.", category: "Supplies", icon: Package },
  { id: 6, title: "HC Verma Physics Vol 1 & 2", price: 400, condition: "Good", seller: "Neha K.", category: "Books", icon: BookOpen },
]
    ...
    <DashboardLayout title="Marketplace">
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-muted-foreground">Buy and sell books, electronics, and college essentials</p>
          <Button className="gap-2 shadow-md" onClick={() => router.push("/marketplace/add")}>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredProducts.map((product) => {
            ...
            const colorClass = categoryColors[product.category] || "from-slate-500/10 to-slate-600/5"
            const IconComponent = product.icon || ShoppingBag
            
            return (
              <Card key={product.id} className={`border-border/40 bg-gradient-to-br ${colorClass} transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 group`}>
                <CardHeader className="pb-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 space-y-1 min-w-0">
                      <CardTitle className="text-base line-clamp-2 leading-snug">{product.title}</CardTitle>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary" className={`${style.bg} ${style.text} border-0 gap-1`}>
                      {style.icon && <Sparkles className="size-3" />}
                      {product.condition}
                    </Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">Sold by <span className="font-medium text-foreground">{product.seller}</span></p>
                  <div className="flex gap-2">
                    <Button 
                      variant="outline" 
                      size="sm" 
                      className="flex-1 gap-1.5 bg-background/80 border-border/50"
                      onClick={() => router.push(`/marketplace/chat/${product.id}`)}
                    >
